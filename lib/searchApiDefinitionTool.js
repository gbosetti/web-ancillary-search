...
SearchApiDefinitionTool.prototype.initPopupHighlightingListener = function() { 
	...
	this.popupHighlightingListener = function(e) {		
		...
		if(me.menu.firstChild.firstChild == e.target.firstChild){
			me.loadCssFiles();
			me.triggerNode = e.target.triggerNode.parentElement;
			me.highlightDomElement(e.target.triggerNode.parentElement);
		}
		//Get the selection
...
SearchApiDefinitionTool.prototype.loadSearchEngineSubMenus = function() { 
	...
	this.createResultsFilterMenu();
	this.createResultsSorterMenu();
}
...
SearchApiDefinitionTool.prototype.createResultsSorterMenu = function() { 

	var item = this.createSearchEngineSubMenu({
		id: 'ways-extract-search-result-sorter', 
		label: 'Sorter' //this.locale('results_filter')
	});
	var me = this;
	item.onclick = function(evt){
		//me.createPropertyTemplate(evt);
...
SearchApiDefinitionTool.prototype.createResultsFilterMenu = function() { 

	var item = this.createSearchEngineSubMenu({
		id: 'ways-extract-search-result-filter', 
		label: this.locale('results_filter')
	});
	var me = this;
	item.onclick = function(evt){
		//me.createPropertyTemplate(evt);
...
SearchApiDefinitionTool.prototype.saveSearchElement = function(target, key){
	...
	var man = this;
	this.changeSidebarUrl("./search-api-configuration.html");
	this.onceInSidebar(function(){
		//console.log(target);
