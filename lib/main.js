...
var woaLocale = require("sdk/l10n").get;
...
ExtensionManager.prototype.loadPopupEventsInSidebar = function(menupopup, sidebarWindow){
	...
	man.sidebarListener = function(e) {
		if(e.eventPhase<3) {
			man.hideContextMenuItems(menupopup.childNodes);
...
ExtensionManager.prototype.enableCTemplatesSelection = function() {
	...
	this.hideContextMenuItems(utils.getMostRecentBrowserWindow().document.getElementById("contentAreaContextMenu").childNodes);
...
ExtensionManager.prototype.createChildContextMenu = function(data) {
    ...
    item = data.doc.getElementById(data.id);

    //If the menu doesn't exist,it's created
    if(item == undefined) {
        //Creates the element
        item = data.doc.createElement("menuitem");
        //The item should be created inside a menupopup
        item.setAttribute("id", data.id);
        //item.id = data.id;

        //Get the parent and add it
        if(data.menu.tagName == 'menupopup'){ // menupopup
            parent = data.menu;
            console.log("PARENT OPT 1", parent);
        } else { // menu
            parent = data.menu.getElementsByTagName("menupopup")[0];

            if(parent == null) {

            	var mpp = data.doc.createElement("menupopup");
            	data.menu.appendChild(mpp);
            	parent = mpp; //.parentElement;

            	//menupopup 

            	console.log("PARENT OPT 2", parent);
            } else console.log("PARENT OPT 3", parent);
        }
    ...
    if(data.callback) item.onclick = function(e) { data.callback(e) };
    return item;
}
ExtensionManager.prototype.createParentContextMenu = function(data) { 

	var newmenu;
	if(data.menu && data.menu != undefined && data.menu != null){ //If a menu is specified

		//Gets the element,if it already exists
		newmenu = data.doc.getElementById(data.id);

		//If the menu doesn't exist,it's created
		if(!newmenu || newmenu == undefined || newmenu == null) {

			//Creates the element
			newmenu = data.doc.createElement("menu"); 
			//Gets/creates the menupopup. The menu element could not have a menupopup
			var menupopup;
			if(data.menu.tagName == 'menupopup'){ //If it's the menupopup
				menupopup = data.menu;
			}
			else if(data.menu.getElementsByTagName("menupopup").length == 0){ //If it has no menupopup
				menupopup = data.menu.appendChild(data.doc.createElement("menupopup"));
			}
			else { //If it's parent of a menupopup
				menupopup = data.menu.getElementsByTagName("menupopup")[0];
			}
			//Appends the menu 
		...
		newmenu.innerHTML = "";
		//Adds a menupopup to the node,if it doesn't have one
		//console.log("CREATING DATA MENU, LENGTH: " + data.label, newmenu.getElementsByTagName("menupopup").length);
		if(newmenu.getElementsByTagName("menupopup").length == 0){
			var newmenupopup = data.doc.createElement("menupopup");
			console.log("****** newmenupopup: ", newmenu);
		...
		if(data.className) newmenu.className = data.className;
		newmenu.id = data.id;
...
ExtensionManager.prototype.attachToActiveWebpage = function(config){
	...
	var wrk = tabs.activeTab.attach({
		contentScriptFile: this.getAsDataResources(config.jsFiles) 
	});
...
var keep_extension_database = woaLocale('keep_extension_database');
exports.onUnload = function (reason) {
	if(reason == 'uninstall' || reason == 'upgrade' || reason == 'downgrade' || 'disable'){
		
		let file = FileUtils.getFile("ProfD", ["jetpack", "woa@lifia.info.unlp.edu.ar", "woa.sqlite"]);
		if(file.exists()){
			if(!utils.getMostRecentBrowserWindow().confirm(keep_extension_database)){
				file.remove(0);
...
var ext = new ExtensionManager(woaLocale); 
	ext.devMode();
*/
