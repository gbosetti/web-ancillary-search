...
SearchTool.prototype.getDataForVisualization = function(iframe, callback){
	...
	me.pageWorker.port.on("notifyDomForResultsExtraction", function(data){
		...
		var domForResults = NsIDomParser.parseFromString(data.textContent, "text/html"); //.wrappedJSObject;
		
		//var win = require('sdk/window/utils').getMostRecentBrowserWindow().content;
		var data = {
...
SearchTool.prototype.getGoodreadsApi = function(first_argument) {
...
class ImgWrapper extends DomElemWrapper { 
	...
	getBase64Image(img) {
		try{
			...
			var chromeWindow = require('sdk/window/utils').getMostRecentBrowserWindow().content;
